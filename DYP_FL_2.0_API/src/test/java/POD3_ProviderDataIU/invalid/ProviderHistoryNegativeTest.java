...
/**
 * POD3_ProviderDataIU - ProviderHistoryNegativeTest (Invalid Scenarios)
 * 
 * Purpose: This test class validates the getProviderHistory API endpoint for Internal User (IU) scenarios.
 * It tests the ability to handle invalid parameters with valid authentication and parameters.
 * 
 * API Endpoint: /api/internal-user-api/v1/providerhistory
 * Method: GET
 * Authentication: Internal User (IU) Token
 * 
 * Test Scenarios:
 * - Invalid getProviderHistory with invalid parameters
 * - getProviderHistory with missing required parameters
 * - Core getProviderHistory error handling validation
 * 
 * Expected Results:
 ...
 * 
 * This is a core API that provides fundamental getProviderHistory information with error handling
 */
public class ProviderHistoryNegativeTest extends BaseTest {
    ...
    /**
     * Test: Invalid getProviderHistory with Invalid Parameters
     * 
     * Description: Tests the unsuccessful getProviderHistory with invalid authentication.
     ...
     */
    @Test
    public void testProviderHistoryWithInvalidParameters() throws Exception {
        // Get test data
        ...
        logTestInfo("🎯 Testing getProviderHistory API with Invalid Parameters");
        ...
        .when()
            .get(basePath + "/providerhistory")
        .then()
            .statusCode(anyOf(is(400), is(422), is(500)))
            .body("success", anyOf(equalTo(false), nullValue()));
            
        logTestPass(" getProviderHistory API negative test completed successfully");
    }
}