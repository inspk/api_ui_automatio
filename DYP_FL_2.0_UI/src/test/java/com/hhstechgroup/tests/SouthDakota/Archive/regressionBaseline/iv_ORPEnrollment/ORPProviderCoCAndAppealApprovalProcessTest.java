...
/**
 * This class contains a test which uses a data provider to get a Coc denied of an ORP provider and appeals and approve it.
 */
public class ORPProviderCoCAndAppealApprovalProcessTest extends BaseClassUI {

    String enrollmentType = Data.ORP_APPEAL_APPLICATION;
    ExcelWrite excel = new ExcelWrite(providerInfoSheet, 0);

    /**
     * This method logs in as an approved ORP provider. Clicks on Coc tab, gets list from Coc table and clicks on
     * the one which contains tracking number of denied Coc. Then clicks on appeal button. Fills in appeal section,
     * and uploads document and submits.Then logs in as an internal user, changes the status of appeal
     * to "Approved". For verification checks the status to verify it has changed to "Approved" and writes
     * the data into ProviderInfo.xlsx
     * @param firstName
     * @param lastName
     * @param emailID
     * @param taxonomy
     ...
     * @throws Exception
     */
    @Test(dataProvider = "getORPProviderNameWithCOCStatusDenied",
            dataProviderClass = DataProviderForProviderInfo.class,dependsOnGroups = "ORPCocDeny")
    public void createORPEnrollmentCoCProcessApproveAppealTest(String testEnvironment,String firstName, String lastName, String emailID,String taxonomy, String npi, String trackingNumber) throws Exception {
