...
/**
 * This test class changes the status of a Group enrollment from 'Pending Review' to 'Approved'.
 */
public class GroupProviderApproveEnrollmentTest extends BaseClassUI {

    ExcelWrite excel = new ExcelWrite( providerInfoSheet,0);

    /**
     * This test method retrieves a Group enrollment with 'Pending Review' status from ProviderInfo.xlsx, logs into DyP
     * as an Internal User, executes a search for the enrollment, approves the enrollment, verifies the status of the
     * enrollment is 'Active', logs out as the Internal User and writes the status of the enrollment to
     * ProviderInfo.xlsx.
     *
     * @param firstName
     * @param lastName
    ...
    public void approveGroupEnrollment (String firstName, String lastName, String email, String taxonomy, String npi, String trackingNumber) throws Exception {
     ...
     */
      ...
      //  enrollmentPageInternalUser.pendingReviewFlowConfigaration(firstName,lastName, "PENDING REVIEW", trackingNumber);
        ...
        ProviderInformation.updateProviderData(providerInfoSheet,  Data.groupApplication,firstName,lastName,Data.APPLICATION_STATUS_ACTIVE);
        excel.readExcel();
    }
}