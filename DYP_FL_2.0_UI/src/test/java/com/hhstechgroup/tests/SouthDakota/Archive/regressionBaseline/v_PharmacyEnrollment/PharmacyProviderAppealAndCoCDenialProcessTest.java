...
/**
 * This class contains a test which uses a data provider to get a Coc denied of a pharmacy provider and appeals and deny it.
 */
public class PharmacyProviderAppealAndCoCDenialProcessTest extends BaseClassUI {

    String enrollmentType = Data.PHARMACY_APPEAL_APPLICATION;
    ExcelWrite excel = new ExcelWrite(providerInfoSheet, 0);

    /**
     * This method logs in as an approved pharmacy, clicks on Coc tab, gets list from Coc table and clicks on
     * the one which contains tracking number of denied Coc. Then clicks on appeal button. Fills in appeal section
     * , uploads document and submits.Then logs in as an internal user and changes the status of appeal to "Denied".
     * For verification checks the status to verify that the has changed to "Denied" and writes the data into ProviderInfo.xlsx
     * @param firstName
     * @param lastName
     * @param emailID
     * @param taxonomy
     ...
     * @throws Exception
     */
    @Test(dataProvider = "getPharmacyProviderNameWithCOCStatusDenied",
            dataProviderClass = DataProviderForProviderInfo.class)
    public void createPharmacyEnrollmentCoCProcessDenialAppealTest(String testEnvironment,String firstName, String lastName, String emailID,String taxonomy, String npi, String trackingNumber) throws Exception {
