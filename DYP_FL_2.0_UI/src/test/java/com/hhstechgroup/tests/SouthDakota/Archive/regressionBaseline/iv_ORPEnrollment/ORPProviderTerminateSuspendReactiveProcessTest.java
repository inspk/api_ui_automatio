...
/**
 * This class contains a test which uses a data provider to get an approved ORP provider, Suspends, terminates and reactive.
 */
public class ORPProviderTerminateSuspendReactiveProcessTest extends BaseClassUI {
    String enrollmentType ="ORP" ;
    ExcelWrite excel = new ExcelWrite( providerInfoSheet,0);

    /**
     * This method logs in as an internal user, searches for the provider and verifies the status is active. Then clicks on
     * the provider and suspends.For verification it searches for provider and verifies the status is "Suspended".
     * Then clicks on the provider, reactives by filling the date and the reason. For verification checks the status to see if
     ...
     */
    @Video
    @Test(dataProvider="getORPProviderNameWithStatusApproved", dataProviderClass = DataProviderForProviderInfo.class,  priority = 1)
    public void suspendAndReactiveProvider(String  testEnvironment,String firstName, String lastName, String emailID,String trackingNum) throws IOException {
        ...
        enrollmentPageInternalUser.logOut();
        ProviderInformation.updateProviderData(providerInfoSheet, Data.orpApplication,firstName,lastName,Data.applicationStatusActive);
        excel.readExcel();
    }
     ...
     */
    @Video
    @Test(dataProvider="getORPProviderNameWithStatusApproved", dataProviderClass = DataProviderForProviderInfo.class, priority = 2)
    public void terminateReactiveProvider(String firstName, String lastName, String emailID,String trackingNum) throws IOException {
        homePage.signInToApp(internalUserEmail, internalUserPassword);
        enrollmentPageInternalUser.clickProvidersTab();
        enrollmentPageInternalUser.navigateToPrvdrTabAndSearchForEnrollmentbyProvider(enrollmentType,firstName+" "+lastName);
        enrollmentPageInternalUser.verifyTheStatusOfApplication(Data.applicationStatusActive);
